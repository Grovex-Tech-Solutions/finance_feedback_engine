...
class FinanceFeedbackEngine:
    ...
    async def analyze_asset_async(
            ...
            if self.unified_provider and (
                ...
                if latest_candle and latest_epoch is not None:
                    ...
                    if current_epoch is None or latest_epoch >= current_epoch:
                        ...
                        market_data["open"] = float(latest_candle.get("open", market_data.get("open", 0.0)))
                        market_data["high"] = float(latest_candle.get("high", market_data.get("high", 0.0)))
                        market_data["low"] = float(latest_candle.get("low", market_data.get("low", 0.0)))
        ...
        # Generate decision using the decision engine's AI manager
        prompt = f"Generate a trading decision for asset {asset_pair} using the provided market data."
        # Query the AI manager (async) for a decision
        try:
            decision = await self.decision_engine.ai_manager.query_ai(
                prompt,
                asset_pair=asset_pair,
                market_data=market_data,
            )
        except Exception as e:
            logger.error("AI decision generation failed: %s", e, exc_info=True)
            # Fallback to a safe HOLD decision
            decision = {"action": "HOLD", "confidence": 0.0, "reason": f"AI error: {e}"}
        record_decision_latency("ai_manager", asset_pair, time.perf_counter() - _decision_start)
        update_decision_confidence(asset_pair, decision.get("action", "HOLD"), decision.get("confidence", 0))

    async def get_portfolio_breakdown_async(self) -> Dict[str, Any]:
        """
        Get complete portfolio breakdown asynchronously.

        This is the async version of the portfolio breakdown method,
        used by the trading loop agent for position awareness and
        duplicate-entry guards.

        Returns:
            Dictionary with portfolio breakdown including:
            - total_value_usd: Total portfolio value in USD
            - futures_value_usd: Futures account value
            - spot_value_usd: Spot USD/USDC value
            - platform_breakdowns: Per-platform breakdown (coinbase, oanda)
            - futures_positions: List of active futures positions
            - futures_summary: Futures account summary
            - per_platform_cash: Cash balances per platform

        Raises:
            TradingError: If trading platform is not initialized
            AttributeError: If platform doesn't support portfolio breakdown
        """
        if not self.trading_platform:
            logger.warning("Trading platform not initialized for portfolio breakdown")
            return {
                "total_value_usd": 0.0,
        ...
        try:
            # Check if platform has the method
            if not hasattr(self.trading_platform, "get_portfolio_breakdown"):
                logger.warning(
                    f"Platform {type(self.trading_platform).__name__} "
                    "does not implement get_portfolio_breakdown"
                )
                return {
                    "total_value_usd": 0.0,
                    "futures_value_usd": 0.0,
            ...
            # Call the platform's portfolio breakdown method
            # Run in executor if it's a sync method
            portfolio = await asyncio.get_event_loop().run_in_executor(
                None, self.trading_platform.get_portfolio_breakdown
            )

            logger.info(
                "Portfolio breakdown fetched: $%.2f total across %d assets",
                portfolio.get("total_value_usd", 0),
                portfolio.get("num_assets", 0),
            )
            ...
            logger.error(
                f"Failed to fetch portfolio breakdown: {e}",
                exc_info=True
            )
            # Return empty portfolio on error (don't block trading)
            return {
                "total_value_usd": 0.0,
                "futures_value_usd": 0.0,
                "spot_value_usd": 0.0,
    ...
    def get_portfolio_breakdown(self) -> Dict[str, Any]:
        """
        Get complete portfolio breakdown (synchronous wrapper).

        This is a synchronous wrapper around get_portfolio_breakdown_async(),
        used by code that runs in an executor or doesn't have async context.
        ...
        try:
            ...
            if loop.is_running():
                # If loop is already running, we're likely in an executor
                # Just call the async method directly and let caller handle it
                # This shouldn't happen in normal usage but fallback to direct call
                logger.warning(
                    "get_portfolio_breakdown() called from running event loop - "
                    "use get_portfolio_breakdown_async() instead"
                )
                # Return empty portfolio to avoid blocking
                return {
                    "total_value_usd": 0.0,
                    "futures_value_usd": 0.0,
                    "spot_value_usd": 0.0,
                ...
                # Loop exists but not running, safe to run_until_complete
                return loop.run_until_complete(self.get_portfolio_breakdown_async())
        except RuntimeError:
            ...
            logger.error(
                f"Failed to fetch portfolio breakdown (sync): {e}",
                exc_info=True
            )
            # Return empty portfolio on error (don't block trading)
            return {
